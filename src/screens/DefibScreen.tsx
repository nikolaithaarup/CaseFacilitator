...
export function DefibScreen({
  ...
  const JOULE_OPTIONS = [10, 15, 20, 30, 50, 70, 85, 100, 120, 150, 200] as const;
  ...
  async function shareEkgStrip() {
    if (!caseEkgImg) {
      Alert.alert("Ingen EKG", "Der er ikke noget EKG-billede til denne case endnu.");
      return;
    }
    ...
    try {
      ...
      await onLogDefib("DEFIB_STRIP_SHARED", { ekgKey: rhythmKey }, "EKG shared");
    } catch (e: any) {
      console.warn(e);
  ...
  async function measureEtco2() {
    ...
    setTimeout(async () => {
      ...
      try {
        await onLogDefib("DEFIB_ETCO2", { etco2: etBase, rf: rfBase }, "EtCO2 + RF");
      } catch {}
    }, 3200);
  }
  ...
  async function measureBs() {
    ...
    setTimeout(async () => {
      ...
      const bs = isFiniteNumber(bs0) ? clamp(jitterFloat(Number(bs0), 0.2), 0, 40) : null;
  ...
  async function measureTemp() {
    ...
    setTimeout(async () => {
      ...
      const temp = isFiniteNumber(t0) ? clamp(jitterFloat(Number(t0), 0.1), 25, 45) : null;
  ...
  async function runEkg12() {
    ...
    if (!caseEkgImg) {
      Alert.alert("Ingen EKG", "Der er ikke et EKG-billede til denne case endnu.");
      return;
    }
  ...
  async function startChargeToSelected() {
    ...
    chargeTimeoutRef.current = setTimeout(async () => {
      ...
      try {
        await onLogDefib("DEFIB_CHARGE", { charged: true, joules: j }, `Charge ${j}J`);
      } catch {}
  ...
  async function doShock() {
    ...
    setTimeout(async () => {
      ...
      try {
        await onLogDefib("DEFIB_SHOCK", { joules: joulesDelivered }, `Shock ${joulesDelivered}J`);
      } catch {}
    }, 800);
  }
  ...
  async function doDischarge() {
    ...
    // clear charged state
    const hadCharge = chargedJoules != null || defibBusy === "CHARGE" || isCharged;
    setChargedJoules(null);
    ...
    onSetDisplay(hadCharge ? "Afladt (discharged)" : "Ingen opladning at aflade");

    try {
      await onLogDefib("DEFIB_CHARGE", { charged: false, joules: 0, discharged: true }, "Discharge");
    } catch {}
  }
    ...
    <SafeAreaView edges={["left", "right", "bottom"]} style={[styles.container, { paddingTop: 0 }]}>
        ...
        >
          ...
          <View style={{ marginTop: 12 }}>
            <TouchableOpacity
              style={[
                styles.button,
                { backgroundColor: "#374151", borderRadius: 16, paddingVertical: 14 },
              ]}
      ...
      <View style={{ flex: 1, flexDirection: isLandscape ? "row" : "column", gap: 12 }}>
        ...
        <View style={{ width: railWidth }}>
          ...
          <RailButton
            label={isCharged ? `CHARGED (${chargedJoules}J)` : showJoulePicker ? "CHARGE (start)" : "CHARGE"}
            busyKey="CHARGE"
            tone="warn"
          ...
          >
            ...
            {showEkg12Progress ? (
              <View style={{ marginTop: 12 }}>
                ...
                >
                  {Math.max(0, Math.ceil((EKG12_DURATION_MS * (1 - ekg12Progress)) / 1000))}s
                </Text>
              </View>
            ) : null}
            ...
            {!showEkg12Progress && showJoulePicker ? (
              <View style={{ marginTop: 12 }}>
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          setSelectedJoules(j);
                          chargingTargetRef.current = j;
                          onSetDisplay(`Selected ${j}J. Press CHARGE to start.`);
                        }}
                        style={{
                          paddingVertical: 10,
                          paddingHorizontal: 14,
                          borderRadius: 14,
                          backgroundColor: selected ? "#f59e0b" : "rgba(255,255,255,0.08)",
                          borderWidth: 1,
                ...
                <Text style={{ color: "rgba(255,255,255,0.65)", fontSize: 12, marginTop: 2 }}>
                  Valgt: <Text style={{ color: "white", fontWeight: "900" }}>{selectedJoules}J</Text>{" "}
    ...
    >
      {defibBusy === "CHARGE"
        ? `${Math.max(
            0,
            Math.ceil(
              (jouleDurationMs(chargingTargetRef.current) * (1 - chargeProgress)) / 1000
            )
              ...
              {hasUserRunEkg ? (
                <View style={{ marginTop: 8 }}>
                  <Text style={{ color: "rgba(255,255,255,0.80)" }}>
                    EKG ready:{" "}
                    <Text style={{ color: "white", fontWeight: "900" }}>{defibEkgKey}</Text>
                  </Text>

                  <TouchableOpacity
                    style={[styles.button, { marginTop: 10, backgroundColor: "#374151" }]}
                ...
                <Text style={{ color: "rgba(255,255,255,0.55)", marginTop: 8, flexWrap: "wrap" }}>
                  ...
                  {caseEkgImg ? "" : " (No EKG image available for this case yet.)"}
                </Text>
              )}
            </View>
          </View>
